...
- **習得技術**：REST API設計やテスト駆動開発など、実務で頻繁に使用される技術

### 2. 独自で追加実装した内容
- **機能拡張**：講師からの依頼に応じて、2つの新たな機能（後述💡マーク）を実装。DB設計から見直しが必要であったため、CRUD処理をすべて再実装。
...
## 機能一覧
※💡：機能拡張（検索条件の追加、申し込み状況機能の追加）

| 機能       | 詳細                            |
|:---------|:----------------------------------------------------------------|
| 受講生詳細の一覧検索（機能拡張に伴い削除）  | すべての**受講生詳細**を取得します     |      |
...
## 力をいれたところ
### 🔶仕様変更：クライアントが使いやすく、かつ保守性の高い仕様の追求
![移行図](https://github.com/user-attachments/assets/91cc806f-d2eb-4929-8827-60952c60bf33)

以下のプロセスで仕様変更の提案を行い、講師・メンターとの双方向のやり取りを経て採用され、独自の実装で取り組むことになりました。

**1. ver1に対する疑問**  <br>
 - 受講生コース情報と申込状況が1：1の関係である意味は？
...
|---|---|---|
| **ver1** | 受講中・受講修了の2つの日付情報のみの登録に留まっている | カラムを増やしたいという要望が来たときにどっちのDBに追加すべきか分かりづらい |
| **ver2** | 4つの申し込み状況すべての日付情報が管理可能になる<br>それに伴い、ビジネス課題に対するデータ管理がしやすくなる | DBの責務に沿って、カラムの追加・削除に柔軟に対応できるようになり、保守性が高まる |
