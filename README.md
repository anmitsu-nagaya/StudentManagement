...
- [はじめに：本プロジェクトの設計思想と開発プロセス](はじめに：本プロジェクトの設計思想と開発プロセス)
...
# はじめに：本プロジェクトの設計思想と開発プロセス
本アプリは、「実務で通用する設計力と自走力」を証明することを目的として開発しました。

## 1. 「基礎の徹底」と「AIとの付き合い方」
360時間の開発過程において、バックエンド（Java/Spring Boot）を主軸に据え、以下のステップで「自力」を磨きました。
- バックエンド: 教材のなぞり書きではなく、要件から自力で実装 → レビュー → 模範解答との比較によるリファクタリングを徹底し、**「なぜこの設計が推奨されるのか」を試行錯誤** しました。
- フロントエンド: JavaScriptからTypeScriptへ独学で移行。AIを活用して設計例を提示させつつ、出力されたコードを一行ずつ検証・修正して実装することで、**AIを活用した開発フローへの適応力**を養いました。

## 2. インフラ：抽象化に頼らない「仕組み」の理解
本プロジェクトのインフラ（AWS/EC2）は、あくまで「アプリケーションが動く基盤の全体像」を掌握するための選択です。<br>
PaaS（Vercel/Render等）による自動化に頼らず、Linuxでの環境構築やネットワーク設定を自力で行うことで、アプリ層とインフラ層の境界線を理解しました。<br>
CI/CD（GitHub Actions）を導入し、開発から公開までのパイプラインを構築。「継続的な品質向上」を意識した開発サイクルを経験することに主眼を置いています。<br>

## 3. ソリューションとしての設計：ver1 → ver2 への進化
単にコードを書くのではなく、「現場の課題をどう解くか」という視点を重視しました。<br>
初期設計（ver1）の不備に自ら気づき、現役エンジニアに相談の上再設計を断行。<br>
「退会分析」や「業務自動化」といった実運用のユースケースを想定したDB構造にすることで、「技術を手段としてビジネスの課題を解決する」というエンジニアとしての基本姿勢を反映させています。<br>
- [詳しくはsection9へ](#9-工夫した点力を入れた点)
...
- **得られた価値**
  - 単なる管理システムから、「リードタイム（検討期間）の可視化」や「退会傾向の分析」が可能な、経営判断に寄与するデータ基盤へと昇華させました。<br>
...
## 🔶業務フローに即したユースケース設計
「システムのために人間が動く」のではなく、「業務を支えるためのシステム」として設計しました。
- **ヒューマンエラーの排除（オートメーション）**
    - ステータス変更時の日付自動刻印やUUIDの自動採番を実装。管理者の入力負荷を軽減するとともに、手入力によるデータの不整合を未然に防いでいます。
- **データの永続価値（論理削除）**
  - 「削除して終わり」ではなく、退会者の傾向をKPIとしてモニタリングできるよう論理削除を採用。将来的なマーケティング活用というビジネス要件をシステムに反映させています。

## 🔶保守性を担保する境界線（疎結合）の設計
AI時代のスピード感に対応するため、修正箇所を局所化できる構造を追求しました。
- **データの責務分離（DTO ⇔ Domain ⇔ Data）**
  - APIの入出力と内部ロジック、DB構造を独立させ、変換（Converter）を挟むことで、一方の変更が他方に影響しない疎結合な設計を徹底しました。

- **テスト戦略の最適化** <br>
テストの肥大化を防ぐため、コントローラ・DTO・内部ロジックでテストの責務を分離。
...
これにより、「どこで何が保証されているか」が明確な、メンテナンス性の高いコードベースを構築しました。
