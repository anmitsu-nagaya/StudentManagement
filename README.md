...
Java/Spring Bootを教えるプログラミングスクールで学んだ内容を、学習成果として形にするために作成しました。<br/>
このプロジェクトは以下の2つの内容が組み合わさっています。
...
### 2. 独自で追加実装した内容
- **機能追加**：講師からの依頼に応じて、2つの新たな機能（後述💡マーク）を実装。DB設計から見直しが必要であったため、CRUD処理をすべて再実装。
...
## 機能一覧
※💡：機能追加（検索条件の追加、申し込み状況機能の追加）

| 機能       | 詳細                            |
|:---------|:----------------------------------------------------------------|
| 受講生詳細の一覧検索（機能追加に伴い削除）  | すべての**受講生詳細**を取得します     |      |
...
以下のプロセスで仕様変更の提案を行い、講師・メンターとの双方向のやり取りを経て採用され、独自の実装で取り組みました。
...
**4. ver2を講師・メンターに提案後、承認を得て独自の実装へ**  <br>
