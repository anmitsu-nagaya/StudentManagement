...
## 作成背景
- Java / Spring Boot の学習成果を形にするために作成しました。
- 開発を進めるにあたってプルリクエストによるレビューを受けました。
- 実務で頻繁に使用されるREST API設計やテスト駆動開発を採用しました。
- 一部の機能は講師からの要件定義に従い、解説なしで独自実装しました（後述の💡マーク）。
- バックエンドAPIの動作確認用として、TypeScript/Reactでフロントエンドを実装しました（基礎文法を独学後、AIを活用して実装）。
...
<details><summary>プロセス詳細</summary>
